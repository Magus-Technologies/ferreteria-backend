...
class KardexInventarioRepository implements KardexRepositoryInterface
{
    public function getPaginated(array $filters = [], int $perPage = 50, int $page = 1): array
    {
        $query = KardexInventario::query();
    ...
    public function create(array $data): KardexInventario
    {
    ...
    public function getPeriodTotals(array $filters = []): array
    {
        ...
        $totals = $query
            ->selectRaw('
                COALESCE(SUM(CASE WHEN tipo != "compra" THEN cant_ingreso ELSE 0 END), 0) as total_entradas,
                COALESCE(SUM(CASE WHEN tipo != "compra" THEN cant_salida ELSE 0 END), 0) as total_salidas
            ')
            ->first();
