...
class NotaDebitoException extends Exception
{
    ...
    {
        return new self("La venta no es válida para generar nota de débito: {$razon}", 422);
    }
    ...
    public static function notaDebitoNoEncontrada(string $notaDebitoId): self
    {
        return new self("La nota de débito con ID {$notaDebitoId} no fue encontrada", 404);
    }

    public static function notaDebitoNoEditable(string $estado): self
    {
        return new self("La nota de débito no puede editarse en estado: {$estado}", 422);
    }

    public static function notaDebitoNoEnviable(string $razon): self
    {
        return new self("La nota de débito no puede enviarse: {$razon}", 422);
    }

    public static function montoInvalido(string $razon): self
    {
        return new self("El monto de la nota de débito es inválido: {$razon}", 422);
    }
    ...
    {
        return new self("Error al guardar la nota de débito: {$detalle}", 500);
    }
}
