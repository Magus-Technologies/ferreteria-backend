    ...
    public function update(Request $request, string $id)
    {
        ...
        return DB::transaction(function () use ($id, $validated) {
            $entrega = \App\Models\Entrega::with([
                'detalles',
                'detalles.unidadDerivadaVenta',
                'tipoEntrega:id,codigo',
                'tipoDespacho:id,codigo',
                'estadoEntrega:id,codigo',
                'quienEntrega:id,codigo',
            ])->findOrFail($id);
            $venta = Venta::find($entrega->venta_id);

            // Helpers: códigos de catálogo de la entrega (tabla nueva usa FKs).
            $codDe = fn ($cat) => ($c = optional($cat)->codigo) instanceof \BackedEnum ? $c->value : $c;
            ...
            if (! empty($validated['productos_entregados'])) {
                $detallesExistentes = $entrega->detalles->keyBy('unidad_derivada_venta_id');
                ...
                foreach ($validated['productos_entregados'] as $detalleActualizado) {
                    ...
                    $cantidadProgramadaOriginal = (float) $detalleExistente->cantidad;
                    ...
                    if ($esPlaceholderOmitido) {
                        ...
                        $detalleExistente->cantidad = $maxPermitido;
                    ...
                    if ($cantidadNueva > 0) {
                        $eventDetalles[] = [
                            'entrega_detalle_id' => $detalleExistente->id,
            ...
            if (! empty($eventDetalles) && in_array($estadoEventoNuevo, ['ec', 'en'])) {
                ...
                foreach ($eventDetalles as $det) {
                    DetalleEntregaEvento::create([
                        ...
                        'entrega_detalle_id' => $det['entrega_detalle_id'],
            ...
            $estadoAnterior = $codDe($entrega->estadoEntrega);
            if ($estadoNuevo === 'en' && $estadoAnterior !== 'en') {
                ...
                if (empty($eventDetalles)) {
                    $eventoEc = EntregaEvento::where('entrega_id', $entrega->id)
                        ->where('estado', 'ec')
                        ->latest()
                        ->first();
                    if ($eventoEc) {
                ...
                if ($requiereConsumoDiferido) {
                    foreach ($entrega->detalles as $detalle) {
                        ...
                        if ($cantidadEntregada <= 0 && $tipoEntregaCod === 'rt') {
                            $cantidadEntregada = (float) $unidadDerivadaVenta->cantidad_pendiente;
                            $detalle->cantidad = $cantidadEntregada;
                            $detalle->save();
                        }
                        $cantidadPendiente = (float) $unidadDerivadaVenta->cantidad_pendiente;
            ...
            // Construir datos de actualización para la tabla nueva (FKs de catálogo).
            $updateData = [];
            foreach ([
            ...
            if (($validated['estado_entrega'] ?? null) === 'en') {
                $updateData['fecha_ejecutada'] = now();
            }
            // Traducir códigos legacy → FK de catálogo.
            if (isset($validated['estado_entrega'])) {
                $updateData['estado_entrega_id'] = \App\Models\EstadoEntrega::where('codigo', $validated['estado_entrega'])->value('id');
            }
            if (isset($validated['tipo_entrega'])) {
                $updateData['tipo_entrega_id'] = \App\Models\TipoEntrega::where('codigo', $validated['tipo_entrega'])->value('id');
            }
            if (isset($validated['tipo_despacho'])) {
                $updateData['tipo_despacho_id'] = \App\Models\TipoDespacho::where('codigo', $validated['tipo_despacho'])->value('id');
            }
            if (isset($validated['quien_entrega'])) {
                $updateData['quien_entrega_id'] = \App\Models\QuienEntrega::where('codigo', $validated['quien_entrega'])->value('id');
            }

            $entrega->update($updateData);

            // Recalcular cantidad_pendiente de la venta de forma ABSOLUTA desde la
            // tabla nueva (idempotente). Reemplaza al recalcularEstado() legacy
            // (que servía al modelo de grupo madre/hijas, inexistente en la nueva).
            app(\App\Services\Entrega\EntregaSyncLegacyService::class)->sincronizar($entrega->fresh());
